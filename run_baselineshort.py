...
# -*- coding: utf-8 -*-
"""
LLMREI-Short baseline 测试入口：运行 ReqElicitGym 评估 -> 输出对话和指标。
...
def build_parser():
    parser = argparse.ArgumentParser(description="Baseline Short (LLMREI-Short)")
    parser.add_argument(
        "--mode",
...
def main():
    ...
    if mode == "top3":
        ...
        if top3_path.exists():
            ...
            temp_data_path = temp_dir / "test_baseline_short_top3.json"
        ...
        temp_dir = _REPO_ROOT / "output" / "temp"
        temp_dir.mkdir(parents=True, exist_ok=True)
        temp_data_path = temp_dir / "test_baseline_short_sample.json"
        with open(temp_data_path, "w", encoding="utf-8") as f:
            json.dump(selected, f, ensure_ascii=False, indent=2)
        data_path = str(temp_data_path)
    else:  # full
    ...
    evaluation_result_path = str(
        Path(metrics_dir) / f"baselineshort_{model}_{mode}_{time_suffix}.json"
    )
    conversation_result_path = str(
        Path(conversation_dir) / f"baselineshort_{model}_{mode}_{time_suffix}.json"
    )

    from ReqElicitGym.config import ReqElicitGymConfig
    from ReqElicitGym.env import ReqElicitGym
    from baseline.short_interviewer import Interviewer
    ...
    print("=" * 60)
    print("Baseline Short (LLMREI-Short) 配置")
    print("=" * 60)
